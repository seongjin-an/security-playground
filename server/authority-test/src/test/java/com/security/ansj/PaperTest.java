...
public class PaperTest extends WebIntegrationTest{
    ...
    private Paper paper1 = Paper.builder()
            .paperId(1L).title("시험지1").tutorId("tutor1")
            .studentIds(Arrays.asList("user1")).state(Paper.State.PREPARE)
            .build();

    private Paper paper2 = Paper.builder()
            .paperId(2L).title("시험지2").tutorId("tutor1")
            .studentIds(Arrays.asList("user2")).state(Paper.State.PREPARE)
            .build();

    @DisplayName("1. user1이 시험지 리스트 조회함")
    @Test
    void test_1(){
    ...
    @DisplayName("3. user2 라도 출제중인 시험지에는 접근할 수 없음")
    @Test
    void test_3(){
        paperService.setPaper(paper2);
